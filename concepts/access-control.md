...
- [访问控制 Access Control](#访问控制-access-control)
  ...
  - [2 Access Control 访问控制](#2-access-control-访问控制)
    ...
    - [2.4 Access Control Policy 访问控制模型](#24-access-control-policy-访问控制模型)
...
### 2.4 Access Control Policy 访问控制模型
...
* **实现方式**：MAC模型更为结构化、更为严格，并且**基于安全标签系统**。用户被赋予一个安全许可(秘密、绝密、机密等)，并且数据以相同的方式分类。安全许可和分类数据存储在安全标签内，安全标签则绑定在特定的主体和客体上。当系统收到一个对客体的访问请求时，它会根据主体的安全许可、客体的分类以及系统的安全策略来做出决策。**主体如何访问客体的规则由安全政策确定，由管理员配置，由操作系统实施，由安全技术支持。**
