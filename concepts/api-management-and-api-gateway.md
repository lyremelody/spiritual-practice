...
- [API管理与API网关](#api管理与api网关)
  ...
  - [2 API网关](#2-api网关)
    ...
    - [2.3 API网关有哪些主要功能？](#23-api网关有哪些主要功能)
  - [3 微服务模式：API Gateway / Backends for Frontends](#3-微服务模式api-gateway--backends-for-frontends)
    - [3.1 背景](#31-背景)
    - [3.2 问题](#32-问题)
    - [3.3 需求](#33-需求)
    - [3.4 解决方案：API网关模式](#34-解决方案api网关模式)
    - [3.5 API网关模式变种：Backends for Frontends](#35-api网关模式变种backends-for-frontends)
...
* 你希望**保护API免受过度使用和滥用**，因此选择了身份验证服务和速率限制。
* 你**想了解人们如何使用你的API**，因此添加了分析和监控工具。
* 如果你已**有货币化API**，则还要连接到计费系统。
...
### 2.3 API网关有哪些主要功能？
API网关的工作因实施不同而异。一些常见的功能包括：身份验证、路由、速率限制、计费、监控、分析、策略、警报和安全防护。
TODO：待完善
...
### 3.5 API网关模式变种：Backends for Frontends
此模式的一个变体是前端模式的后端。它为每种客户端定义了一个单独的API网关。

<div align=center><img src="./api-management-and-api-gateway/bffe.png"></div>

在此示例中，存在三种客户端：Web 应用程序、移动应用程序和外部第 3 方应用程序。共有三种不同的 API 网关。每一个都为其客户端提供一个 API。
...
7. [API网关能做什么？，RedHat](https://www.redhat.com/zh/topics/api/what-does-an-api-gateway-do)
8. [Pattern: API Gateway / Backends for Frontends，microservices.io](https://microservices.io/patterns/apigateway.html)