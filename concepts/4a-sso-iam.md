...
## 2 访问控制
信息安全领域，访问控制包括 Authentication(认证)、Authorization(授权)和Audit(审计)。
...
#### 4.1.2 OAuth 2.0 解决问题的关键思想：角色分离、授权访问、访问控制
